...
{ 
    ...
    { 
        ...
        cout<<"Enter meaning:\t"; 
        cin>>temp->meaning; 
        temp->left=temp->right=NULL; //Initializes child pointers to NULL. 
 
            root=insert(root,temp); //Inserts the new node into the AVL tree. 
...
{ 
    if(cur==NULL) 
    { 
        return temp; //Recursive base case: If position is found, return new node. 
    } 
    if(temp->key<cur->key) 
    { 
...
{ 
    int bal; 
    bal=dif(temp); //returns balance factor (height(left) - height(right)) 
...
{ 
   ...
   else //Go left or right based on comparison 
   { 
       node *l = cur->left; 
       node *r = cur->right; 
       delete cur; 
       if ( !r ) //check if right child exists 
           return l;  
       node *m=r; 
